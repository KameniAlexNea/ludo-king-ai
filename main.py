...
class SimpleAI:
    ...
    def make_decision(self, game_context: dict) -> int:
        ...
        print(f"{self.name}: Moving token {best_move['token_id']} (strategic value: {best_move['strategic_value']:.1f})")
    ...
    while not game.game_over and turn_count < turn_limit:
        ...
        if move_result['success']:
            ...
            if move_result['captured_tokens']:
                ...
                print(f"Captured: {[(t['player_color'], t['token_id']) for t in captured]}")
...
def demonstrate_ai_interface():
    ...
    if context['valid_moves']:
        ...
        move_result = game.execute_move(current_player, first_move['token_id'], dice_value)
        print("Move result:")
