...
SAVE_STEPS=${SAVE_STEPS:-500000}
DEVICE=${DEVICE:-cpu}
MAX_TURNS=${MAX_TURNS:-500}
OPPONENT_STRATEGY=${OPPONENT_STRATEGY:-}
FIXED_COLOR=${FIXED_COLOR:-RED}
PI_NET_ARCH=${PI_NET_ARCH:-"128 256 128"}
VF_NET_ARCH=${VF_NET_ARCH:-"256 512 256"}
N_ENVS=${N_ENVS:-16}
...
if [[ -n "${OPPONENT_STRATEGY}" && "${OPPONENT_STRATEGY,,}" != "none" ]]; then
    TRAIN_ARGS+=(--opponent-strategy "${OPPONENT_STRATEGY}")
fi
