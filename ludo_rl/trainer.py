...
class LudoRLTrainer:
    ...
    def __init__(self, game_data_file: str = None, state_saver_dir: str = "saved_states"):
        """
    ...
    def calculate_reward(self, game_data: Dict, next_game_data: Dict = None) -> float:
        ...
        if chosen_move_idx < len(valid_moves):
            ...
            if (best_move and 
                chosen_move.get('token_id') == best_move.get('token_id')):
                reward += 5.0
    ...
    def create_training_sequences(self) -> List[List[Tuple]]:
        ...
        for i, game_data in enumerate(self.game_data):
            ...
            action = self.encoder.encode_action(
                game_data['chosen_move'],
                game_data['game_context']['valid_moves']
            )
    ...
    def train(self, epochs: int = 1000, target_update_freq: int = 100, 
              save_freq: int = 100, model_save_path: str = "ludo_dqn_model.pth") -> Dict:
        """
        Train the RL agent.
        ...
        for epoch in range(epochs):
            ...
            batches_per_epoch = max(1, len(self.agent.memory) // (self.agent.batch_size * 10))
            ...
            if epoch % 100 == 0:
                ...
                print(f"Epoch {epoch}/{epochs} - Loss: {avg_loss_str}, "
