...
class LudoStateEncoder:
    ...
    def _calculate_state_dim(self):
        ...
        valid_moves_features = self.max_tokens * 6  # token_id, move_type, safety, capture, strategic_value, finishing
    ...
    def encode_state(self, game_data: Dict) -> np.ndarray:
        ...
        state[idx + 1] = min(current_situation['consecutive_sixes'], 3) / 3.0  # normalize
        state[idx + 2] = min(current_situation['turn_count'], 200) / 200.0  # normalize turn count
        ...
        for opponent in opponents[:self.num_players-1]:
            state[idx] = (self.max_tokens - opponent['tokens_active']) / self.max_tokens  # estimated home tokens
        ...
        for i in range(self.max_tokens):
            if i < len(valid_moves):
                move = valid_moves[i]
                state[idx] = move['token_id'] / (self.max_tokens - 1)  # normalize token ID
                ...
                state[idx + 4] = min(move['strategic_value'], 30.0) / 30.0  # normalize strategic value
    ...
    def _encode_player_tokens(self, state: np.ndarray, start_idx: int, tokens: List[Dict], player_offset: int) -> int:
