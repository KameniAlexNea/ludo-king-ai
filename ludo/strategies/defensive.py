...
class DefensiveStrategy(Strategy):
    ...
    def decide(self, game_context: Dict) -> int:
        ...
        if home_moves:
            best_home = max(home_moves, key=lambda m: (
                m["target_position"],
        ...
        if safe_moves:
            filtered = [m for m in safe_moves if self._is_within_defensive_threat(threat_map[m["token_id"]])]
            if filtered:
        ...
        if active < StrategyConstants.DEFENSIVE_MIN_ACTIVE_TOKENS or pressure:
            ...
            if exit_move and self._is_within_defensive_threat(threat_map.get(exit_move["token_id"], (0, 99))):
                return exit_move["token_id"]

        # 7. Reposition away from higher threat squares
        reposition_candidates = [m for m in moves if self._reposition_improves(m, threat_map, game_context)]
    ...
    def _compute_threats(self, moves: List[Dict], ctx: Dict) -> Dict[int, Tuple[int, int]]:
        ...
        opponent_positions = [
            t["position"]
            for p in ctx.get("players", []) if p["color"] != current_color
            for t in p["tokens"]
    ...
    def _filter_block_friendly(self, moves: List[Dict], blocks: List[int]) -> List[Dict]:
        out: List[Dict] = []
        for mv in moves:
            ...
            if dest_block or (source_block and dest_block) or (source_block and dest_block):
                out.append(mv)
            else:
    ...
    # --- Capture Logic ---
    def _choose_safe_capture(self, moves: List[Dict], threat_map: Dict[int, Tuple[int, int]]) -> int | None:
    ...
    # --- Repositioning ---
    def _reposition_improves(self, mv: Dict, threat_map: Dict[int, Tuple[int, int]], ctx: Dict) -> bool:
        tid = mv["token_id"]
        ...
        return landing_threat[0] < current_threat[0] or landing_threat[1] > current_threat[1]

    # --- Selection Helpers ---
    def _select_safest(self, moves: List[Dict], threat_map: Dict[int, Tuple[int, int]]) -> int | None:
        if not moves:
            return None
