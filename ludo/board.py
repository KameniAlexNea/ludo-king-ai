...
class Position:
    ...
    def __post_init__(self):
        ...
        colored_squares = {
            1: PlayerColor.RED.value, 9: PlayerColor.RED.value,
            14: PlayerColor.GREEN.value, 22: PlayerColor.GREEN.value,
            27: PlayerColor.YELLOW.value, 35: PlayerColor.YELLOW.value,
            40: PlayerColor.BLUE.value, 48: PlayerColor.BLUE.value
        }
...
class Board:
    ...
    def can_move_to_position(self, token: Token, target_position: int) -> Tuple[bool, List[Token]]:
        """
        ...
        if self.is_position_safe(target_position, token.player_color):
            ...
            same_color_tokens = [t for t in tokens_at_target if t.player_color == token.player_color]
            opponent_tokens = [t for t in tokens_at_target if t.player_color != token.player_color]
    ...
    def execute_move(self, token: Token, old_position: int, new_position: int) -> List[Token]:
        """
    ...
    def get_blocking_positions(self, player_color: str) -> Set[int]:
        ...
        for position, tokens in self.token_positions.items():
            ...
            if len(player_tokens) >= 2 and not self.is_position_safe(position, player_color):
