...
def strategy_tournament():
        ...
        for j, strategy2 in enumerate(strategies):
            if i < j:  # Avoid duplicate matchups
                ...
                for game_num in range(games_per_matchup):
                    ...
                    # Assign strategies
                    game.players[0].set_strategy(StrategyFactory.create_strategy(strategy1))
                    game.players[1].set_strategy(StrategyFactory.create_strategy(strategy2))
                ...
                print(f"  Result: {strategy1.upper()} {s1_wins} - {s2_wins} {strategy2.upper()}")
    ...
    print(f"{'Rank':<4} {'Strategy':<12} {'Wins':<6} {'Losses':<7} {'Draws':<6} {'Win Rate':<8}")
    print("-" * 55)
    
    for rank, stats in enumerate(strategy_stats, 1):
        print(f"{rank:<4} {stats['name'].upper():<12} {stats['wins']:<6} {stats['losses']:<7} {stats['draws']:<6} {stats['win_rate']:.1f}%")
...
def demonstrate_strategic_thinking():
    ...
    if context['valid_moves']:
        ...
        for i, move in enumerate(context['valid_moves']):
            print(f"  {i+1}. Move Token {move['token_id']} from {move['current_position']} to {move['target_position']}")
            print(f"     Type: {move['move_type']}, Safe: {move['is_safe_move']}, Value: {move['strategic_value']:.1f}")
        print()
        ...
        for strategy_name in strategies_to_test:
            ...
            chosen_move = next((m for m in context['valid_moves'] if m['token_id'] == choice), None)
            
            if chosen_move:
                print(f"{strategy_name.upper():<10}: Token {choice} (to pos {chosen_move['target_position']}) - {strategy.description}")
            else:
...
def compare_strategies_detailed():
    ...
    for strategy_name in strategies:
        ...
        for game_num in range(num_games):
            ...
            # Track game stats
            game_stats = {'turns': 0, 'captures': 0, 'finished': 0, 'risky': 0, 'safe': 0}
                ...
                if context['valid_moves']:
                    ...
                    if current_player.color == PlayerColor.RED:
                        if move_result.get('captured_tokens'):
                            game_stats['captures'] += len(move_result['captured_tokens'])
                        if move_result.get('token_finished'):
                            game_stats['finished'] += 1
                    
                    if move_result.get('game_won') and current_player.color == PlayerColor.RED:
    ...
    print("-" * 70)
    print(f"{'Strategy':<12} {'Win Rate':<10} {'Avg Turns':<10} {'Captures':<10} {'Finished':<10}")
    print("-" * 70)
    ...
    for strategy_name in strategies:
        ...
        print(f"{strategy_name.upper():<12} {win_rate:<10.1f}% {avg_turns:<10.1f} {stats['tokens_captured']:<10} {stats['tokens_finished']:<10}")
