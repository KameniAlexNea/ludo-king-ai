...
def feature_extraction_example():
    ...
    def extract_features(game_context):
        ...
        features = {
            ...
            "opponent_finished_tokens": sum(
                opp["tokens_finished"] for opp in opponents
            ),
            "opponent_active_tokens": sum(opp["tokens_active"] for opp in opponents),
            # Strategic features
            "max_strategic_value": max(
                [move["strategic_value"] for move in valid_moves], default=0
            ),
            "avg_strategic_value": (
                sum(move["strategic_value"] for move in valid_moves) / len(valid_moves)
                if valid_moves
                else 0
            ),
        }
