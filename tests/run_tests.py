...
class TestRunner:
    ...
    def _run_specific_tests(self, test_type, verbose):
        ...
        try:
            ...
            runner = unittest.TextTestRunner(
                stream=stream, verbosity=2 if verbose else 1
            )
    ...
    def _print_test_results(self, test_type, stats, result, success):
        ...
        print(
            f"  Success rate: {((stats['total'] - stats['failures'] - stats['errors']) / max(stats['total'], 1) * 100):.1f}%"
        )
    ...
    def _print_overall_summary(self, stats, success):
        ...
        print(
            f"Overall success rate: {((stats['total'] - stats['failures'] - stats['errors']) / max(stats['total'], 1) * 100):.1f}%"
        )
            ...
            for scenario_name, context_data in scenarios:
                ...
                for strategy_name in strategies:
                    ...
                    selected_move = next(
                        (
                            m
                            for m in game_context["valid_moves"]
                            if m["token_id"] == decision
                        ),
                        {"move_type": "unknown"},
                    )
                    ...
                    print(
                        f"{scenario_name:<15} {strategy_name.upper():<12} {decision:<8} {move_type:<12} {self._get_move_reasoning(strategy_name, move_type)}"
                    )
        ...
        return {
            ...
            "player_state": {
                "tokens_home": len(
                    [
                        t
                        for t in context_data.game_state.players[0].tokens
                        if t.position == -1
                    ]
                ),
                "tokens_active": len(
...
def main():
    ...
    parser.add_argument(
        "--type",
        choices=["all", "unit", "framework", "integration", "performance"],
        default="all",
        help="Type of tests to run",
    )
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    parser.add_argument(
        "--analysis", action="store_true", help="Run strategy behavior analysis"
    )
