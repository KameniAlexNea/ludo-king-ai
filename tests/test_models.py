...
class TestScenarioBuilder:
    ...
    def build_all_strategy_tests(self) -> List[StrategyTestCase]:
        ...
        scenarios = [
            ...
            (
                TestDataFactory.create_multi_choice_scenario(),
                TestScenario.MULTI_OPTION_CHOICE,
            ),
    ...
    def _get_expected_behavior(
        self, strategy: str, scenario: TestScenario
    ) -> ExpectedBehavior:
    ...
    def _get_expected_token(
        self, strategy: str, context: DecisionContext
    ) -> Optional[int]:
    ...
    def _get_reasoning(self, strategy: str, scenario: TestScenario) -> str:
        ...
        reasoning_map = {
            (
                "killer",
                TestScenario.TOKEN_CAPTURE,
            ): "Aggressive strategy prioritizes capturing opponents",
            (
                "winner",
                TestScenario.ENDGAME,
            ): "Goal-oriented strategy focuses on finishing tokens",
            (
                "defensive",
                TestScenario.DEFENSIVE_PLAY,
            ): "Safety-first approach moves to secure positions",
            (
                "cautious",
                TestScenario.MULTI_OPTION_CHOICE,
            ): "Ultra-conservative play avoids all risks",
            (
                "optimist",
                TestScenario.GAME_START,
            ): "Forward-thinking strategy advances aggressively",
        }

        return reasoning_map.get(
            (strategy, scenario), f"{strategy} strategy standard behavior"
        )
