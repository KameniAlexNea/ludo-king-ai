...
from tests.test_models import (
    ExpectedBehavior, TestDataFactory, TestScenarioBuilder
)
...
class LudoTestFramework(unittest.TestCase):
    ...
    def assertValidMove(self, token_id: int, valid_moves: List[Dict], msg: str = ""):
        ...
        self.assertIn(token_id, valid_token_ids, 
                     f"Token {token_id} not in valid moves {valid_token_ids}. {msg}")
    
    def assertStrategyBehavior(self, strategy: Strategy, context: Dict, 
                              expected_behavior: ExpectedBehavior, msg: str = ""):
        """Assert that strategy exhibits expected behavior."""
        decision = strategy.decide(context)
        valid_moves = context.get('valid_moves', [])
        
        # Validate decision is legal
        self.assertValidMove(decision, valid_moves, f"Strategy {strategy.name} made invalid move")
        
        # Check behavior alignment
        selected_move = next((m for m in valid_moves if m['token_id'] == decision), None)
        self.assertIsNotNone(selected_move, f"Selected move not found for token {decision}")
        
        behavior_checks = {
            ExpectedBehavior.CAPTURE_OPPONENT: lambda m: m.get('move_type') == 'capture',
            ExpectedBehavior.MOVE_TO_SAFETY: lambda m: m.get('is_safe_move', False),
            ExpectedBehavior.ADVANCE_CLOSEST: lambda m: m.get('move_type') in ['normal', 'advance'],
        ...
        if expected_behavior in behavior_checks:
            ...
            if expected_behavior != ExpectedBehavior.RANDOM_CHOICE:
                behavior_match = any(check_func(move) for move in valid_moves if move['token_id'] == decision)
                if not behavior_match and len([m for m in valid_moves if check_func(m)]) > 0:
                    self.fail(f"Strategy {strategy.name} did not exhibit {expected_behavior.value}. {msg}")
...
class StrategyTestSuite(LudoTestFramework):
    ...
    def test_all_strategies_valid_decisions(self):
        ...
        for test_case in test_cases:
            with self.subTest(strategy=test_case.strategy_name, scenario=test_case.scenario):
                strategy = self.strategy_factory.create_strategy(test_case.strategy_name)
                context = self._convert_context_to_game_format(test_case.input_context)
                
                decision = strategy.decide(context)
                valid_moves = context.get('valid_moves', [])
                
                self.assertValidMove(decision, valid_moves, 
    ...
    def test_killer_strategy_captures(self):
        ...
        capture_moves = [m for m in game_context['valid_moves'] if m.get('move_type') == 'capture']
        if capture_moves:
            self.assertIn(decision, [m['token_id'] for m in capture_moves],
    ...
    def test_defensive_strategy_safety(self):
        ...
        selected_move = next((m for m in game_context['valid_moves'] if m['token_id'] == decision), None)
        self.assertIsNotNone(selected_move, "Defensive strategy must select a valid move")
    ...
    def test_winner_strategy_finishing(self):
        ...
        finish_moves = [m for m in game_context['valid_moves'] if m.get('move_type') == 'finish']
        if finish_moves:
            self.assertIn(decision, [m['token_id'] for m in finish_moves],
    ...
    def test_strategy_consistency(self):
        ...
        for strategy_name in ["killer", "winner", "defensive", "cautious"]:
            ...
            self.assertEqual(len(unique_decisions), 1, 
    ...
    def test_random_strategy_variability(self):
        ...
        if len(game_context['valid_moves']) > 1:
            ...
            self.assertGreater(len(unique_decisions), 1, 
                             "Random strategy should show variability")
    ...
    def _convert_context_to_game_format(self, test_context) -> Dict:
        ...
        return {
            ...
            'player_state': {
                'tokens_home': len([t for t in test_context.game_state.players[0].tokens if t.position == -1]),
                'tokens_active': len([t for t in test_context.game_state.players[0].tokens if 0 <= t.position <= 55]),
                'tokens_finished': len([t for t in test_context.game_state.players[0].tokens if t.position >= 56])
            },
            'opponents': [
                {
                    'color': player.color,
                    'tokens_finished': len([t for t in player.tokens if t.position >= 56]),
                    'tokens_active': len([t for t in player.tokens if 0 <= t.position <= 55]),
...
class GameFlowTestSuite(LudoTestFramework):
    ...
    def test_first_move_requires_six(self):
        ...
        self.assertGreater(len(valid_moves), 0, "Should have moves available with rolling 6")
        # All moves should be exit_home type
        exit_moves = [m for m in valid_moves if m.get('move_type') == 'exit_home']
        self.assertEqual(len(valid_moves), len(exit_moves), "All moves should be exit_home type")
    
    def test_strategic_game_completion(self):
        """Test complete strategic game from start to finish."""
        # Set up players with strategies
        self.game.players[0].set_strategy(self.strategy_factory.create_strategy("balanced"))
        self.game.players[1].set_strategy(self.strategy_factory.create_strategy("optimist"))
        ...
        while not self.game.game_over and turn_count < max_turns:
            ...
            if context['valid_moves']:
                ...
                move_result = self.game.execute_move(current_player, selected_token, dice_value)
        ...
        if self.game.game_over:
            ...
            self.assertLessEqual(turn_count, max_turns, f"Game took too long: {turn_count} turns")
...
class PerformanceTestSuite(LudoTestFramework):
    ...
    def test_strategy_decision_speed(self):
        ...
        for strategy_name in ["killer", "winner", "defensive", "balanced", "cautious", "optimist", "random"]:
            strategy = self.strategy_factory.create_strategy(strategy_name)
            
            start_time = time.time()
            for _ in range(100):  # Run 100 decisions
                decision = strategy.decide(game_context.copy())
                self.assertIsNotNone(decision, f"Strategy {strategy_name} should make decision")
            end_time = time.time()
            
            avg_time = (end_time - start_time) / 100
            self.assertLess(avg_time, 0.01, f"Strategy {strategy_name} too slow: {avg_time:.4f}s per decision")
    ...
    def test_tournament_scalability(self):
        ...
        for i, strategy1 in enumerate(strategies):
            for strategy2 in strategies[i+1:]:
                for game_num in range(games_per_matchup):
                    ...
                    game.players[0].set_strategy(self.strategy_factory.create_strategy(strategy1))
                    game.players[1].set_strategy(self.strategy_factory.create_strategy(strategy2))
                    
                    winner = self._play_quick_game(game, max_turns=50)
                    total_games += 1
                    
                    if winner:
                        winner_strategy = strategy1 if winner.color == PlayerColor.RED else strategy2
                        loser_strategy = strategy2 if winner_strategy == strategy1 else strategy1
        ...
        total_outcomes = sum(r['wins'] + r['losses'] + r['draws'] for r in results.values())
        self.assertEqual(total_outcomes, total_games * 2, "Win/loss accounting should balance")
