...
def _draw_stacked_tokens(
    ...
    tokens_at_pos: List[Tuple[PlayerColor, TokenView]],
    center_x: int,
    center_y: int,
    base_radius: int,
    show_ids: bool = True,
...
def _home_column_positions_for_color(color: PlayerColor) -> Dict[int, Tuple[int, int]]:
    """Map home column indices (52..56) to board coordinates; 57 is the final finish.

    GameConstants.HOME_COLUMN_SIZE = 6 spans 52..57 inclusive, but per spec the last
...
def draw_board(
    tokens: Dict[PlayerColor, List[TokenView]], show_ids: bool = True
) -> Image.Image:
    """
    ...
    for color, tlist in tokens.items():
        for tk in tlist:
            ...
            if state is TokenState.HOME:
                ...
                state is TokenState.HOME_COLUMN
                and GameConstants.HOME_COLUMN_START
                <= pos
                <= GameConstants.FINISH_POSITION - 1
            ):
                # Home column tokens
            ...
            elif state is TokenState.FINISHED:
