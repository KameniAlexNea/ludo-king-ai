...
def play_rl_vs_strategies(model_path: str, num_games: int = 10):
        ...
        try:
            ...
            if game.winner:
                if game.winner == game.players[0]:  # RL agent won
                    ...
                    for i, opponent_strategy in enumerate(available_opponents):
                        if i + 1 < len(game.players) and game.winner == game.players[i + 1]:
    ...
    print(f"   RL Agent wins: {results['rl_wins']} ({results['rl_wins']/max(results['total_games'], 1)*100:.1f}%)")
    ...
    try:
        ...
        if game_context['valid_moves']:
            ...
            for i, move in enumerate(game_context['valid_moves']):
                print(f"     {i}: Token {move['token_id']} - {move['move_type']} "
        ...
        if best_move:
            print(f"📋 Best strategic move: Token {best_move['token_id']} "
...
def main():
    ...
    if results and results["total_games"] > 0:
        ...
        if rl_win_rate > 0.4:
            ...
            print("   📉 Needs improvement. Try different hyperparameters or more data.")
